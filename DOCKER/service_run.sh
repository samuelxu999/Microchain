...
if [[ "" == $2 ]]; then
	CONTAINER_NAME="microchain-node"
...
if  [ "start" == "$OPERATION" ]; then
	...
	if [ "" == "$BOOT_STRAP" ]; then
		...
		CMD_Line="python3 /home/docker/app/WS_Server.py --blockepoch 3 --pauseepoch 3 --phasedelay 2 -p $PORT --rp $RPC_PORT --firstnode --save_state 60"
	else
		echo "Run as ENF node to contact: $BOOT_STRAP"
		CMD_Line="python3 /home/docker/app/WS_Server.py --blockepoch 3 --pauseepoch 3 --phasedelay 2 -p $PORT --rp $RPC_PORT --refresh_neighbors 15 --bootstrapnode $BOOT_STRAP"
	fi
	## run ENF service app
